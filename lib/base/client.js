...
module.exports = class Client extends Base {
	...
	send(queue, data) {
		return this.engine.push(this.getNames(queue), this.stringify({
	...
	invoke(queue, data) {
		if (!this.pullQueue) {
			this.pullQueue = this.getNames(this.options.queueName || this.getUUID())
		...
		const correlationId = this.getUUID()
